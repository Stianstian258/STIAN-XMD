...
zokou({
  ...
  try {
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "THIS IS HAPPINESS XMD TECH NEWS",
          body: "keep enjoying", 
...
╰─────────────────◆ `;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "HAPPINESS XMD HOLY BIBLE",
  ...
  try {
    ...
    if (definition) {
      ...
      await zk.sendMessage(dest, {
        ...
        contextInfo: {
          externalAdReply: {
            title: "HAPPINESS XMD DICTIONARY",
...
zokou({
  ...
  try {
    ...
    const replyText = "```Wait happiness xmd is getting your pair code ❤️💚...```";
    await repondre(replyText);

    // Prepare the API request
    const encodedNumber = encodeURIComponent(arg.join(" "));
    const apiUrl = `https://njabulo-5778.onrender.com/code?number=${encodedNumber}`;
...
Regards ${conf.BOT} `;
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          title: "hαppínєss хmd єlєmєnt ínfσrmαtíσn",
